...
class OntologyApp:
    ...
    def create_widgets(self):
        ...
        prev_frame = ttk.LabelFrame(main_frame, text="Optional: Select Previous Version", padding=10)
        prev_frame.pack(fill=tk.X, pady=5)

        ttk.Button(
            prev_frame,
            text="Browse Previous OWL",
        ...
        ttk.Label(naming_frame, text="Folder Prefix:").pack(side=tk.LEFT)
    ...
    def select_previous_owl_file(self):
        filename = filedialog.askopenfilename(
            title="Select Previous Version OWL File",
            filetypes=[("OWL files", "*.owl"), ("All files", "*.*")]
        )

        if filename:
            self.previous_owl_file.set(filename)
            self.set_status(f"Selected previous version: {Path(filename).name}")
