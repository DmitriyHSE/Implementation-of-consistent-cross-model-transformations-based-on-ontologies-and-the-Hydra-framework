...
def analyze_java_classes(java_file: Path) -> Dict[str, Dict[str, str]]:
    """Анализирует Java файл и возвращает структуру классов с полями и их типами"""
...
def compare_versions(versions: List[Tuple[str, Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    for i in range(1, len(versions)):
        ...
        for cls in old_classes & new_classes:
            ...
            # Проверяем, изменился ли файл класса
...
def generate_report(ontology_name: str, changes: List[Dict[str, Any]]) -> str:
    """Формирует читаемый отчет об изменениях между версиями"""
    if not changes:
...
def collect_java_files(base_dir: Path) -> Dict[str, Any]:
    """Собирает все Java файлы из директории и анализирует их"""
    ontology_data = {}
    ...
    for dir_name in os.listdir(base_dir):
        ...
        if dir_path.is_dir():
            ...
            # Ищем Java файлы в стандартной Maven структуре
